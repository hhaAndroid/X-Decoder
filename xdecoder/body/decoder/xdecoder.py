...
class XDecoder(nn.Module):
    ...
    def __init__(
        ...
        self_attn_mask[:, num_queries:, num_queries:] = torch.triu(torch.ones((1, contxt_len, contxt_len)),
                                                                   diagonal=1).bool()  # caption query only attend with previous token.
        self_attn_mask[:, :num_queries - 1,
        num_queries - 1:num_queries] = True  # object query does not attend with class query.
        self_attn_mask[:, num_queries - 1:num_queries,
        :num_queries - 1] = True  # class query does not attend with object query.
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        if task == 'captioning_infer':
            return self.forward_captioning(x, mask_features, mask=mask, target_queries=target_queries,
        ...
        if self.training and task == 'vlp' and self.task_switch['captioning']:
            ...
            caping_lang_embed = torch.cat([caption['caption_tokens'] for caption in target_vlp], dim=0).transpose(0,
                                                                                                                  1)  # language output
            _caping_lang_embed = caping_lang_embed.detach().clone()
            output = torch.cat((output, _caping_lang_embed),
        ...
        if task == 'vlp':
            out = {'pred_captionings': predictions_captioning[-1],
                   ...
                   'aux_outputs': [{'pred_captionings': x, 'pred_captions': y} for x, y in
                                   zip(predictions_captioning[:-1], predictions_caption[:-1])]}
            return out
        else:
            out = {
    ...
    def forward_captioning(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg',
                           extra={}):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        src = []
    ...
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_boxes, outputs_captions):
        ...
        if self.mask_classification:
            return [
                ...
                for a, b, c, d in
                zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_boxes[:-1], outputs_captions[:-1])
            ]
        else:
